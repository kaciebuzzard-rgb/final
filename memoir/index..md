---
layout: default
title: The Waiting Rooms of My Life
permalink: /memoir/
---

<!-- MEMOIR HERO -->

<section class="memoir-hero">

  <div class="memoir-hero-overlay"></div>

  <div class="wrap memoir-hero-inner">

    <p class="memoir-eyebrow">
      Memoir in Progress
    </p>

    <h1>
      The Waiting Rooms of My Life
    </h1>

    <p class="memoir-intro">
      The Waiting Rooms of My Life is an in-progress memoir about caregiving,
      disability, invisible labor, and the long afterlife of crisis.
      These excerpts explore what love asks of families navigating complex
      systems of care — and what it costs to endure inside them.
    </p>

    <div class="memoir-jump-links">
      <a href="#through-line">The Through-Line</a>
      <a href="#hospital-hallway">The Hospital Hallway Where Time Collapses</a>
      <a href="#invisible-tax">The Invisible Tax of Invisible Labor</a>
    </div>

  </div>

</section>

<!-- MEMOIR EXCERPTS -->

<section class="memoir-excerpts">

  <div class="wrap memoir-wrap">

    <!-- EXCERPT 1 -->

    <article class="memoir-entry" id="through-line">

      <img
        src="/assets/img/memoir-chair.png"
        alt="Empty vinyl chair in hospital waiting room"
        class="memoir-image">

      <p class="memoir-label">
        Excerpt One
      </p>

      <h2>
        The Through-Line
      </h2>

      <div class="memoir-copy">

        <p>There is a through-line in my life.</p>

        <p>
          A small vinyl chair in a PICU room.
          A woman who sits.
          Who refuses to leave.
          Who absorbs impact so others don’t have to.
        </p>

        <p>
          For years, love looked like vigilance.
          Advocacy. Logistics. Fighting systems.
          Staying awake when everyone else slept.
        </p>

        <p>Love looked like endurance.</p>

        <p>With Jim, love looked different.</p>

        <p>It looked like steadiness.</p>

        <p>
          It looked like someone saying,
          “You don’t have to carry this alone,”
          and proving it in ordinary ways —
          checking in, showing up, learning my children,
          remembering the hard dates,
          noticing when I was fading.
        </p>

        <p>
          It looked like a man who wasn’t intimidated by my past.
          Not threatened by my competence.
          Not unsettled by my complexity.
        </p>

        <p>
          It looked like someone who didn’t need me to shrink
          in order to stay.
        </p>

        <p>
          In the space of a few short months,
          we hadn’t just fallen in love.
        </p>

        <p>We had built something.</p>

        <p>
          A partnership.<br>
          A friendship.<br>
          A home I could feel myself resting inside.
        </p>

      </div>

    </article>

    <div class="memoir-divider"></div>

    <!-- EXCERPT 2 -->

    <article class="memoir-entry" id="hospital-hallway">

      <img
        src="/assets/img/memoir-hallway.png"
        alt="Hospital hallway with soft lighting"
        class="memoir-image">

      <p class="memoir-label">
        Excerpt Two
      </p>

      <h2>
        The Hospital Hallway Where Time Collapses
      </h2>

      <div class="memoir-copy">

        <p>
          There are hospital hallways that live in the body long after you leave them.
        </p>

        <p>
          Even now, certain light can take me back there —
          that flat fluorescent glare,
          the kind that erases the hour and makes midnight
          feel exactly like 3 p.m.
        </p>

        <p>
          The air always too cold.
          The chairs always designed for waiting,
          never for resting.
        </p>

        <p>
          The sound of wheels.
          Alarms.
          Footsteps.
          Voices lowered just enough
          to suggest something might be wrong.
        </p>

        <p>
          In those places, time does not move forward.
          It folds in on itself.
        </p>

        <p>
          Five minutes can hold an entire life.
          A single sentence from a doctor
          can split the world into before and after.
        </p>

        <p>
          You learn to live in fragments there.
          Test results.
          Shift changes.
          Intake questions.
          Medication schedules.
        </p>

        <p>
          The unbearable choreography of hope and dread.
        </p>

      </div>

    </article>

    <div class="memoir-divider"></div>

    <!-- EXCERPT 3 -->

    <article class="memoir-entry" id="invisible-tax">

      <img
        src="/assets/img/memoir-paperwork.png"
        alt="Desk with paperwork and laptop representing caregiving administrative work"
        class="memoir-image">

      <p class="memoir-label">
        Excerpt Three
      </p>

      <h2>
        The Invisible Tax of Invisible Labor
      </h2>

      <div class="memoir-copy">

        <p>
          People see the appointment.
          They do not see the architecture behind the appointment.
        </p>

        <p>
          They see the child arrive.
          They do not see the insurance call,
          the rescheduled work,
          the school communication,
          the medication refill,
          the form that needed to be printed,
          signed,
          scanned,
          uploaded,
          and followed up on twice.
        </p>

        <p>
          They see whether you made it.
          They do not see what it cost you to get there.
        </p>

        <p>
          That is the invisible tax of invisible labor —
          not just the work itself,
          but the way it disappears while still extracting from you.
        </p>

        <p>
          Time. Attention. Earnings. Sleep.
          Credibility. Marriage. Identity.
        </p>

        <p>
          Caregiving is often described as emotional labor.
          And it is.
        </p>

        <p>
          But it is also systems labor.
          Administrative labor.
          Anticipatory labor.
        </p>

        <p>
          It is project management under duress,
          performed inside love,
          with consequences for failure that are too high.
        </p>

      </div>

    </article>

  </div>

</section>

<!-- MEMOIR CTA -->

<section class="memoir-cta">

  <div class="wrap memoir-cta-inner">

    <h2>
      For editorial, speaking, or podcast inquiries
    </h2>

    <p>
      Kacie writes and speaks at the intersection of disability advocacy,
      caregiving systems, invisible labor, and lived experience.
    </p>

    <a class="btn" href="mailto:kacie.buzzard@gmail.com">
      Get in Touch
    </a>

  </div>

</section>
